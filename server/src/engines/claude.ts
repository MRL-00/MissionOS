...
import { testCliCommand, trackChildProcess } from "./shared.js";
...
export function formatClaudeExitMessage(
  command: string,
  code: number | null,
  ...
  if (signal === "SIGTERM" || code === 143) {
    return [
      "Claude run was interrupted before it finished.",
      "This usually means the MissionOS server restarted or the Claude process was terminated externally.",
    ].join(" ");
  }
...
function extractToolAnnouncements(content: unknown, state: ClaudeParserState): string[] {
  ...
  for (const [index, block] of content.entries()) {
    const blockType = block && typeof block === "object" && "type" in block ? block.type : undefined;
    if (!block || typeof block !== "object" || !isToolUseType(blockType)) {
      continue;
    }

    const fallbackId = `${String(blockType ?? "tool_use")}:${String("name" in block ? block.name : "tool")}:${index}`;
    const id = "id" in block && typeof block.id === "string" && block.id ? block.id : fallbackId;
...
export function parseClaudeStreamEvent(line: string, state: ClaudeParserState): string | null {
  ...
  if (event.type === "content_block_start" && event.content_block && isToolUseType(event.content_block.type)) {
    const fallbackId = `${String(event.content_block.type)}:${String(event.content_block.name ?? "tool")}`;
    const id =
    ...
    state.announcedToolUseIds.add(id);
    const toolName =
      typeof event.content_block.name === "string" && event.content_block.name
