...
function configCliPath(config: Record<string, unknown>): string {
  return typeof config.cliPath === "string" && config.cliPath.trim() ? config.cliPath.trim() : "claude";
}
...
export const claudeLocalAdapter: AdapterModule = {
  ...
  configFields(): AdapterConfigField[] {
    return [
      { key: "cliPath", label: "CLI path", type: "text", placeholder: "claude", hint: "Path to the Claude CLI binary" },
  ...
  async testConnection(config): Promise<AdapterTestResult> {
    ...
    return { ok: true, message: `Claude CLI ${version.stdout}`, latencyMs: Date.now() - start };
  },
