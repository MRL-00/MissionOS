...
export async function handleClaudeAuth(pathname: string, url: URL, response: ServerResponse): Promise<boolean> {
  if (pathname === "/auth/claude/authorize") {
    const redirectTarget = url.searchParams.get("redirect") ?? "http://localhost:5173/";
    const clientId = process.env.CLAUDE_OAUTH_CLIENT_ID ?? "missing-claude-client-id";
    ...
    redirect(response, `/auth/claude/callback?code=mock-claude-code&state=${encodeURIComponent(state)}&client_id=${encodeURIComponent(clientId)}`);
    return true;
  }

  if (pathname === "/auth/claude/callback") {
    ...
    const tokenId = await storeToken({
      provider: "claude",
      code,
      accessToken: `claude-placeholder-token-${Date.now().toString(36)}`,
      clientId: process.env.CLAUDE_OAUTH_CLIENT_ID ?? url.searchParams.get("client_id") ?? undefined,
      createdAt: Date.now(),
    });
    const redirectTarget = new URL(state.redirectTarget ?? "http://localhost:5173/");
    redirectTarget.searchParams.set("oauth_provider", "claude");
