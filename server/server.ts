...
const REMOTE_OFFICE_URL = process.env.REMOTE_OFFICE_URL?.trim() ?? "";
...
// --- Remote Office Mirror ---
// When REMOTE_OFFICE_URL is set, connect to a production office server's WebSocket
// and mirror all agent state locally. This lets a dev instance show live data
// without needing direct OpenClaw access.

let remoteOfficeWs: WsWebSocket | null = null;
let remoteOfficeReconnectTimer: ReturnType<typeof setTimeout> | null = null;

function startRemoteOfficeMirror(): void {
...
function connectRemoteOffice(wsUrl: string): void {
  const socket = new WsWebSocket(wsUrl);
  ...
  socket.on("message", (data) => {
    try {
      ...
      // Mirror individual agent events
